...
document.addEventListener('keydown', (event) => {
    ...
    if (event.keyCode === upKey || event.keyCode === downKey || event.keyCode === wKey || event.keyCode === sKey) {
...
document.addEventListener('keyup', (event) => {
    if (event.keyCode === upKey || event.keyCode === wKey || event.keyCode === downKey || event.keyCode === sKey) {
        playerSpeed = 0;
    }
});
...
function updateGameScores() {
    ...
    gameResults.forEach((game, index) => {
        ...
        li.textContent = `Game ${index + 1}: Player ${game.playerScore} - Computer ${game.computerScore} | Winner: ${game.winner}`;
